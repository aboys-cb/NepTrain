...
def prepare_md(
    ...
    *,
    ...
    effective_plugin = target.overrides.get("md.plugin_path", plugin_path)
    shards = []
    index = 0
    for frame_index, frame in enumerate(frames):
        for temperature in temperatures:
            ...
            request = {
                ...
                "spin_alpha": float(spin_alpha),
                "spin_seed": int(spin_seed),
                "midpoint_iter": int(midpoint_iter),
                "inference_backend": inference_backend,
                "lmp": lmp,
                "mpiexec": mpiexec,
                "mpi_ranks": int(mpi_ranks),
                "plugin_path": effective_plugin,
            ...
            if operation.kind == "train":
                ...
                result = run_md(
                    MdRequest(
                        ...
                        tdamp=float(request["tdamp"]),
                        pdamp=float(request["pdamp"]),
                        ensemble=request["ensemble"],
                        dump_interval=int(request["dump_interval"]),
                        ...
                        spin_alpha=float(request["spin_alpha"]),
                        spin_seed=int(request["spin_seed"]),
                        midpoint_iter=int(request["midpoint_iter"]),
