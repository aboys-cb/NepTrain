...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Label extxyz structures with a local DeePMD model."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except (DeepmdRunnerError, OSError, ValueError) as error:
        parser.exit(1, f"neptrain-label-deepmd: error: {error}\n")
    return 0
