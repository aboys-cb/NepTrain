    ...
    for catalog_path in catalog_paths:
        ...
        for message in catalog:
            ...
            if not message_text.strip():
                errors.append(
                    f"{relative_path}: untranslated message {message_id!r}"
                )
            elif "fuzzy" in message.flags:
                errors.append(f"{relative_path}: fuzzy message {message_id!r}")
            elif HAN_TEXT.search(message_text):
                errors.append(
                    f"{relative_path}: Chinese remains in English text "
                    f"{message_id!r}"
                )

    if errors:
        raise SystemExit(
            "English translation check failed:\n- " + "\n- ".join(errors)
        )
...
<head>
  ...
  <title>NepTrain Documentation</title>
  <script>
